...
case ${DB_TYPE,,} in
    ...
    postgresql|cockroach)
        ...
        col_names=$( echo ${col_name_sql} | psql --csv -t postgresql://${DB_ARC_USER}:${DB_ARC_PW}@${DB_HOST}/${DB_ARC_USER} | grep -v ${REMOVE_COLS} | paste -s -d, )
        col_names_pk=$( echo ${col_pk_sql} | psql --csv -t postgresql://${DB_ARC_USER}:${DB_ARC_PW}@${DB_HOST}/${DB_ARC_USER} | grep -v ${REMOVE_COLS} | paste -s -d, )
