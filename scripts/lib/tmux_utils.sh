...
tmux_show_errorlog() {
    tmux send-keys -t ${TMUX_SESSION}:0.3 "clear; figlet -t errlog; while [ ! -f ${ARCION_LOG}/${LOG_ID}/error_trace.log ]; do sleep 5; done; tail -f ${ARCION_LOG}/${LOG_ID}/error_trace.log" Enter
}
