...
if [ -f ${SCRIPTS_DIR}/${DSTDB_TYPE}/dst.init.sql ]; then
    echo "Running root"
    cat ${SCRIPTS_DIR}/${DSTDB_TYPE}/dst.init.sql | psql postgresql://${PG_ROOT_USER}:${PG_ROOT_PW}@${DSTDB_HOST}:26257/ 2>&1 | tee /tmp/arcion/${DSTDB_HOST}/dst.init.log
fi

# with the arcsrc user
if [ -f ${SCRIPTS_DIR}/${DSTDB_TYPE}/dst.init.arcsrc.sql ]; then
    echo "Running arcsrc"
    cat ${SCRIPTS_DIR}/${DSTDB_TYPE}/dst.init.arcsrc.sql | psql postgresql://${ARCSRC_USER}:${ARCSRC_PW}@${DSTDB_HOST}:26257/ 2>&1 | tee /tmp/arcion/${DSTDB_HOST}/dst.init.arcsrc.log
fi