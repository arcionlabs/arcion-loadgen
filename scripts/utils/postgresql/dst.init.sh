...
DSTDB_DB_PORT=${DSTDB_DB_PORT:-5432}
...
if [ -f ${SCRIPTS_DIR}/${DSTDB_TYPE}/dst.init.sql ]; then
    cat ${SCRIPTS_DIR}/${DSTDB_TYPE}/dst.init.sql | psql postgresql://${DSTDB_ROOT}:${DSTDB_PW}@${DSTDB_HOST}:${DSTDB_DB_PORT}/ 2>&1 | tee -a $CFG_DIR/dst.init.sh.log
fi

# with the arcsrc user
if [ -f ${SCRIPTS_DIR}/${DSTDB_TYPE}/dst.init.arcdst.sql ]; then
    cat ${SCRIPTS_DIR}/${DSTDB_TYPE}/dst.init.arcdst.sql | psql postgresql://${DSTDB_ARC_USER}:${DSTDB_ARC_PW}@${DSTDB_HOST}:${DSTDB_DB_PORT}/${DSTDB_ARC_USER} 2>&1 | tee -a $CFG_DIR/dst.init.sh.log
fi